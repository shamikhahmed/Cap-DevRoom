    ...
    <Link href="/launch" className="mo-card card-hover" style={{ display: "block", padding: 16, marginBottom: 16, textDecoration: "none" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {top.map((s) => (
            <span key={s.projectId} className={`mo-signal ${SIGNAL_CLASS[s.signal] ?? ""}`} title={`${s.name} ${s.readiness}%`}>
              {s.name.replace("Cap", "")} {s.readiness}%
            </span>
          ))}
          <span className="t-footnote" style={{ color: "var(--accent)", alignSelf: "center" }}>Launch Control ›</span>
        </div>
      </div>
    </Link>
  );
